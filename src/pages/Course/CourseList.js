...
const CreateForm = Form.create()(props => {
  ...
  return (
    ...
    >
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="班课号">
        {form.getFieldDecorator('coursename', {
          rules: [{ required: true, message: "请输入班课号"}],
        })(<Input placeholder="请输入班课号" />)}
      </FormItem>
    </Modal>
  );
});
...
class CourseList extends PureComponent {
  ...
  handleAdd = fields => {
    ...
    dispatch({
      type: 'course/joinCourse',
      payload: fields,
    });
    message.error('请输入正确的班课号！');
  ...
  render() {
    ...
    const content = (
      <div className={styles.pageHeaderContent}>
        <p>
          ...
          以下为您已经加入的班课列表，点击详情可以查看班课信息，点击删除可以删除班课。也可以加入一个新的班课。
        </p>
      </div>
    );
    ...
    return (
      <PageHeaderWrapper title="我的班课" content={content} extraContent={extraContent}>
